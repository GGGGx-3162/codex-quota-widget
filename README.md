...
- Refreshes immediately at launch and when Codex writes new quota data, with a 30-second fallback.
...
1. Download `Codex-Quota-Widget-1.3.2.dmg` from [Releases](https://github.com/GGGGx-3162/codex-quota-widget/releases/latest).
...
- 启动时立即读取；Codex 写入新额度时及时刷新，并以每 30 秒刷新作为兜底。
...
A Windows 10/11 taskbar edition is also available.

- Displays remaining Codex 5-hour and weekly quota beside the taskbar weather widget.
- Refreshes through the local Codex app-server; it does not copy account tokens.
- Chinese and English interface switching from the right-click menu.
